...
- **Novatel INS** publishing IMU on `/gps_na/imu` and odometry on `/localization/global/odom`. The URDF link `novatel_a` is used as both `base_frame` and `imu_frame` in the localization config.
